...
public class MapGenerator : MonoBehaviour
{
    ...
    void Update()
    {
        //Every time we press the left button then it generates a new map
    ...
    List<Coord> GetLine(Coord from, Coord to)
    {
        ...
        //In case that the longest is less that the sorted
        //we are inverted
        if (longest < shortest)
        {
    ...
    //method work by the seed in order to feel the map with random values
    //so we will get a map full of random values and then modify it to generate the map
    void RandomFillMap()
    {
        if (useRandomSeed)
        {
    ...
    void SmoothMap()
    {
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                //In case that is more than 4 tiles around,
                //then that point is a wall otherwise is empty tile space
